...
public class EditModeSpawn : MonoBehaviour
{
    ...
    [SerializeField] bool rotateNormal;
    ...
    void OnScene(SceneView scene)
    {
        ...
        else if (e.type == EventType.KeyDown && e.keyCode == KeyCode.M)
        {
            Vector3 mousePos = e.mousePosition;
            float ppp = EditorGUIUtility.pixelsPerPoint;
            mousePos.y = scene.camera.pixelHeight - mousePos.y * ppp;
            ...
            if (Physics.Raycast(ray, out hit, Mathf.Infinity, mask))
            {
                ...
                else if (_parentObject == hit.collider.gameObject)
                {
                    Selection.gameObjects[0].transform.position = hit.point + new Vector3(0, yOffset, 0);
                    //_placedObjectList.Add(go);
                }
            }
            e.Use();
    ...
    {
        Selection.activeObject = PrefabUtility.InstantiatePrefab(placementObject, _parentObject.transform);
        
        // GameObject go = Instantiate(placementObject, _parentObject.transform);
        GameObject go = Selection.activeObject.GameObject();
        ...
        if (rotateNormal)
        {
            ...
            go.transform.LookAt(hit.normal);
            ...
            go.transform.rotation = Quaternion.LookRotation(hit.point + lookAt, hit.normal);
            */
        }
