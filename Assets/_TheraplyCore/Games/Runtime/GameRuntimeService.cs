...
#pragma warning restore CS0414
        ...
        // Deferred START_GAME for legacy bundled games: command arrives before scene load.
        private StartGameCommand _pendingBundledStartCommand;
        private bool _bundledStartCoroutineActive;
        ...
        public bool NotifyPreparedGameSceneReady(string reasonCode = "PREPARE_GAME_SCENE_READY")
        {
            RefreshSceneBindings();

            var normalizedReasonCode = string.IsNullOrWhiteSpace(reasonCode)
                ? "PREPARE_GAME_SCENE_READY"
                : reasonCode.Trim();
            var reboundActiveGame = false;

            if (!string.IsNullOrWhiteSpace(_activeGameId))
            {
                reboundActiveGame = SetActiveGame(_activeGameId);
            }
            ...
            PublishDevicePresenceUpdateIfPossible(
                ...
                force: true);
            PublishRuntimeStatusIfChanged(normalizedReasonCode);
            return reboundActiveGame;
        }
        ...
        private void HandlePrepareGameCommand(PrepareGameCommand command)
        {
            ...
            {
                // For legacy bundled games the XxxGameModule lives in the scene that hasn't
                // loaded yet — so the registry is empty at this point.
                // LegacyBundledSceneRouter handles the actual scene load; RebuildRegistry()
                // is called after load, well before START_GAME arrives.
                // Allow PREPARE_GAME to proceed if the game is in the simulated catalog.
                EnsureSimulatedContentStatesInitialized();
                ...
                if (!string.IsNullOrWhiteSpace(normalizedGameId) &&
                    _simulatedContentStateByGameId.TryGetValue(normalizedGameId, out var catalogState) &&
                    catalogState != null)
                {
                    _activeGameId = normalizedGameId;
                    Logger.Info($"[GameRuntime] PREPARE_GAME: deferred resolve for bundled game (module loads with scene): {normalizedGameId}");
                }
                else
                {
        ...
        private void HandleStartCommand(StartGameCommand command)
        {
            ...
            {
                // For legacy bundled games PREPARE_GAME is sent simultaneously with START_GAME.
                // The scene (and its XxxGameModule) may not have loaded yet — defer execution.
                if (!string.IsNullOrWhiteSpace(_activeGameId))
                {
                    Logger.Info($"[GameRuntime] START_GAME: registry not ready, deferring for bundled game: {_activeGameId}");
                    _pendingBundledStartCommand = command;
                    if (!_bundledStartCoroutineActive)
                    {
                        _bundledStartCoroutineActive = true;
                        StartCoroutine(DeferredBundledStartCoroutine(_activeGameId));
                    }
                    return; // return without throw → command bus sends ACK
                }
        ...
        private IEnumerator DeferredBundledStartCoroutine(string gameId)
        {
            // For legacy bundled games: START_GAME arrives before LegacyBundledSceneRouter
            // has finished loading the scene. Poll until XxxGameModule is in the registry.
            // Also actively rebuilds the registry each tick so a missing RebuildRegistry()
            // call in LegacyBundledSceneRouter doesn't silently block us.
            float timeout = 6f;
            float rebuildInterval = 0.25f;
            float nextRebuild = 0f;

            while (timeout > 0f)
            {
                yield return null;
                timeout -= Time.unscaledDeltaTime;
                nextRebuild -= Time.unscaledDeltaTime;

                if (!string.Equals(_activeGameId, gameId, StringComparison.OrdinalIgnoreCase))
                {
                    _bundledStartCoroutineActive = false;
                    _pendingBundledStartCommand = null;
                    yield break;
                }

                if (_registryService == null) continue;

                // Re-scan the scene periodically — handles cases where LegacyBundledSceneRouter
                // had a null registry reference and skipped the post-load RebuildRegistry().
                if (nextRebuild <= 0f)
                {
                    _registryService.RebuildRegistry();
                    nextRebuild = rebuildInterval;
                }

                if (_registryService.TryResolve(gameId, out var module) && module != null)
                {
                    _activeGame = module;
                    break;
                }
            ...
            if (_activeGame == null ||
                !string.Equals(_activeGameId, gameId, StringComparison.OrdinalIgnoreCase))
            {
                Logger.Warning($"[GameRuntime] Deferred START_GAME timed out — XxxGameModule not found for: {gameId}. " +
                               $"Ensure the game module component is in the scene and implements IGameModule.");
                _pendingBundledStartCommand = null;
                yield break;
            }

            var cmd = _pendingBundledStartCommand;
            _pendingBundledStartCommand = null;

            if (!TryAuthorizeGameCommandByEntitlement(GameCommandIds.StartGame, _activeGameId, out var reason))
            {
                Logger.Warning($"[GameRuntime] Deferred START_GAME entitlement denied: {reason}");
                yield break;
            }

            TryApplyStartCommandConfig(cmd);

            if (!StartActiveGame())
            {
                Logger.Warning($"[GameRuntime] Deferred START_GAME: StartActiveGame() failed for: {gameId}");
            }
        }
        ...
        private void HandleInstallGameCommand(InstallGameCommand command)
        {
            ...
            // Bundled (legacy) games have no assetbundle — they are always locally available.
            // Skip the entitlement gate entirely and mark them Ready immediately.
            if (_simulatedContentStateByGameId.TryGetValue(normalizedGameId, out var bundledCheck) &&
                bundledCheck != null &&
                string.IsNullOrWhiteSpace(bundledCheck.installedBundlePath))
            {
                bundledCheck.owned = true;
                bundledCheck.installedVersion = string.IsNullOrWhiteSpace(bundledCheck.targetVersion)
                    ? _defaultSimulatedContentVersion
                    : bundledCheck.targetVersion;
                bundledCheck.runtimeStatus = ContentRuntimeStatusValues.Ready;
                bundledCheck.updateRequired = false;
                bundledCheck.lastError = string.Empty;
                bundledCheck.updatedAtUtc = DateTime.UtcNow;
                PersistSimulatedContentStates(InstallReasonCodes.Completed);
                _ = PublishGameInstallStatusAsync(bundledCheck, correlationId, InstallReasonCodes.Completed);
                Logger.Info($"[GameRuntime] INSTALL_GAME: bundled game bypass — immediately Ready: {normalizedGameId}");
                return;
            }
            ...
            // Bundled (legacy) games have no assetbundle — skip simulation entirely.
            // Mark as Ready immediately so mobile can send PREPARE_GAME without delay
            // (no liveness watchdog gap from a fake download routine).
            var bundlePath = string.IsNullOrWhiteSpace(state.installedBundlePath)
                ? string.Empty
            ...
            if (string.IsNullOrWhiteSpace(bundlePath))
            {
                ...
                PersistSimulatedContentStates(InstallReasonCodes.Completed);
                _ = PublishGameInstallStatusAsync(state, correlationId, InstallReasonCodes.Completed);
                Logger.Info($"[GameRuntime] INSTALL_GAME: bundled game — skip simulation, marking Ready: {state.gameId}");
                return;
            }
        ...
        private void ApplyPersistedSimulatedContentStates()
        {
            ...
            try
            {
                ...
                for (var i = 0; i < store.states.Count; i++)
                {
                    ...
                    // For bundled (legacy) games the installedBundlePath is always empty —
                    // they are always available locally regardless of the persisted state.
                    // Overwrite owned/status so a stale 'owned=false' from an old build
                    // never blocks access after a code update.
                    var isBundled = string.IsNullOrWhiteSpace(record.installedBundlePath);
                    state.owned = isBundled ? true : record.owned;
                    state.installedVersion = string.IsNullOrWhiteSpace(record.installedVersion)
                        ? (isBundled ? state.targetVersion : string.Empty)
                    ...
                    state.runtimeStatus = isBundled
                        ? ContentRuntimeStatusValues.Ready
                        : (string.IsNullOrWhiteSpace(record.runtimeStatus)
        ...
        private void EnsureSimulatedContentStatesInitialized()
        {
            ...
            for (var i = 0; i < _simulatedContentCatalog.Count; i++)
            {
                ...
                // For owned bundled games, treat them as already installed (version = targetVersion).
                var installedVersion = entry.owned ? targetVersion : string.Empty;
