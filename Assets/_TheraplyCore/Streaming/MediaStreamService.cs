...
namespace TheraplyCore.Streaming
{
    ...
    /// </summary>
    // [RequireComponent(typeof(Camera))] removed — service lives on the persistent
    // [TheraplyCore] GameObject (DontDestroyOnLoad) and tracks the scene camera
    // via _sourceCamera / SetSourceCamera(). LegacyBundledSceneRouter.OnSceneLoaded
    // re-assigns the camera on every scene change.
    public class MediaStreamService : MonoBehaviour
    {
        ...
        [SerializeField] private int _streamWidth = 854;
        ...
        private bool TryApplyConfiguredVideoBitrate(int bitrateBps, out string reasonCode)
        {
            ...
            try
            {
                ...
                for (var i = 0; i < parameters.encodings.Length; i++)
                {
                    ...
                    encoding.minBitrate = (ulong)(bitrateBps / 4);
        ...
        private int GetEffectiveTargetFps()
        {
            ...
            {
                return Mathf.Min(requestedFps, 30);
            }
