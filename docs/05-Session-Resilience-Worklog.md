...
| R-P6-003 | Implement role/license entitlement model (`THERAPIST_FULL`, `PARENT_PURCHASED_PACKS`) | DONE | Flutter now emits runtime entitlement snapshot in `SESSION_ATTACH` (`entitlementProfile`, role/plan/policy/source + entitled game IDs), Unity enforces the same snapshot in `START_GAME` and simulated `INSTALL_GAME` gates, and command ACK/NACK propagates canonical handler reason codes instead of generic `HANDLER_EXCEPTION` |
| R-P6-004 | Implement Quest game content lifecycle (manifest sync, download, verify, install, activate, rollback) | IN_PROGRESS | Dev simulator now executes explicit lifecycle phases (`SYNCING_MANIFEST`, `DOWNLOADING`, `VERIFYING`, `ACTIVATING`, optional `ROLLING_BACK`) for `SYNC_CATALOG`/`INSTALL_GAME`, persists state to `Application.persistentDataPath/session_resilience/content_delivery_state.json`, and restores on startup for deterministic reconnect/restart behavior; real manifest/download/verify/rollback path remains TODO |
...
| R-P6-006 | Build therapist session-template/program builder with per-game parameter presets | DONE | Added guided-session program contract in therapist settings (`guidedSessionPlanSteps`) with per-step JSON preset payloads, parser/validator (`GuidedSessionPlanEditorCodec`), and editable builder UI in therapist settings dialog |
| R-P6-007 | Build parent one-button guided mode from therapist-defined plan | DONE | Parent quick-start now resolves therapist-defined guided plan step, applies game/preset values automatically, and starts session from one button using the configured plan step |
| R-P6-008 | Define adaptive continuation policy for interrupted/incomplete child sessions | DONE | Added configurable continuation policy (`manual`, `resume_under_recovery_window`, `resume_always`) and evaluator-driven auto-continue path for unfinished sessions before guided start |
| R-P6-009 | Enforce per-child session closure lock in UX + protocol | DONE | Added explicit closure-lock precondition for `START_GAME`/`RESUME_GAME` in Flutter using persisted unfinished session state, with deferred/decision-gate UX and canonical reason `SESSION_CLOSURE_REQUIRED`; Unity `GameCommandBus` now returns `SESSION_CLOSURE_REQUIRED` NACK on start/resume session-mismatch lock conflicts |
| R-P6-010 | Add validation matrix for therapist vs parent flows and entitlement edge cases | DONE | Added Flutter matrix test coverage for therapist vs parent runtime profiles and entitlement edge paths: offline cached launch gate, stale license expiry, partial download/installing, uninstall/not-installed, update-required, and role switch (`flutter_controller/test/role_entitlement_validation_matrix_test.dart`) |
...
- P6 update (2026-02-25, content lifecycle lane): `R-P6-004` remains `IN_PROGRESS`; `GameRuntimeService` now runs explicit simulated lifecycle phases (`SYNCING_MANIFEST`, `DOWNLOADING`, `VERIFYING`, `ACTIVATING`, `ROLLING_BACK`) with verify-failure rollback simulation and persisted state rehydration so install/update/uninstall status survives app reconnect/restart.
...
- P6 update (2026-02-25, entitlement lane): `R-P6-003` is now `DONE`; Flutter sends runtime entitlement snapshot on `SESSION_ATTACH`, Unity enforces entitlement gate for `START_GAME` and simulated `INSTALL_GAME`, and critical command ACK/NACK now surfaces canonical runtime reason codes from handler exceptions.
- P6 update (2026-02-25, validation matrix lane): `R-P6-010` is now `DONE`; added therapist/parent entitlement matrix tests that cover offline cached gate behavior, stale license expiration, partial download/installing state, uninstall/not-installed state, update-required launch block, and role switch profile transition.
- P6 update (2026-02-25, closure lock lane): `R-P6-009` is now `DONE`; mobile now blocks `START_GAME`/`RESUME_GAME` when an unfinished session for the same child exists under a different `sessionId` (reason `SESSION_CLOSURE_REQUIRED`), auto-routes to handoff/deferred UX, and Unity `GameCommandBus` returns `SESSION_CLOSURE_REQUIRED` NACK instead of generic lock conflict for start/resume mismatch.
- P6 update (2026-02-25, guided plan lane): `R-P6-006`, `R-P6-007`, and `R-P6-008` are now `DONE`; therapist settings include a guided-plan builder with per-game JSON presets, parent quick-start consumes therapist-defined plan steps, and continuation behavior is policy-driven (`manual`, `resume_under_recovery_window`, `resume_always`) through `GuidedSessionContinuationEvaluator`.
