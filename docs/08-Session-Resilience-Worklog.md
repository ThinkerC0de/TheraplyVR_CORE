...
- `PARTIAL` `LIC-002` - Admin/system grant path contract added (`entitlement_grants` + `entitlement_grant_requests`) and entitlement login gate now overlays active grants; minimal Flutter operator panel exists behind `ENABLE_ENTITLEMENT_OPS_PANEL=true` (write entitlement + grant/revoke), but full admin workflow/backend policy is still TODO (temporary dev bootstrap option for missing `user_entitlements` is available via `ENABLE_DEV_ENTITLEMENT_BOOTSTRAP=true`).
